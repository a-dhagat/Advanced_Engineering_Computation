...
/* virtual */ void FsLazyWindowApplication::Initialize(int argc,char *argv[])
{
	...
	if (argc == 7) 
	{
		...
		double b = (double)atof(argv[3]);
		double d = (double)atof(argv[4]);
		double p = (double)atof(argv[5]);
		double dz = (double)atof(argv[6]);
		
		MakeNACA4DigitAirfoilWing(mesh, naca, s, b, d, p, dz);

		mesh.SaveBinStl(argv[1]);
	}
}
