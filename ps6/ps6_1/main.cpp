...
protected:
	...
	double d;
	YsVec3 t;
	std::vector<YsVec3> naca_airfoil;
...
/* virtual */ void FsLazyWindowApplication::Initialize(int argc,char *argv[])
{
	...
	double dz = (double)atof(argv[2]);

	if(2<=argc)
	{
		naca_airfoil = MakeNACA4DigitAirfoil(naca,dz);
	}
}
...
/* virtual */ void FsLazyWindowApplication::Draw(void)
{
	...
	int i=0;
	while (i < naca_airfoil.size()-2)
	{
		glVertex3d(naca_airfoil[i][0], naca_airfoil[i][1], naca_airfoil[i][2]);
		glVertex3d(naca_airfoil[i+1][0], naca_airfoil[i+1][1], naca_airfoil[i+1][2]);
		i += 1;
	}

	if (i==naca_airfoil.size()-2)
	{
		glVertex3d(naca_airfoil[i][0], naca_airfoil[i][1], naca_airfoil[i][2]);
		glVertex3d(naca_airfoil[i+1][0], naca_airfoil[i+1][1], naca_airfoil[i+1][2]);
		glVertex3d(naca_airfoil[0][0], naca_airfoil[0][1], naca_airfoil[0][2]);
		glVertex3d(naca_airfoil[i+1][0], naca_airfoil[i+1][1], naca_airfoil[i+1][2]);
	}
	glEnd();
