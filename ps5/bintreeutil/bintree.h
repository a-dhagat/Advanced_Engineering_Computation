...
public:
	...
	NodeHandle Insert(const KeyClass &key,const ValueClass &value)
	{
		...
		if (autoRebalance==true)
			Rebalance(Up(MakeHandle(newNode)));
...
public:
	...
	// {
	// 	pseudoroot = GetNode(RootNode());
	// 	auto tail = pseudoroot;
		
	// 	// auto tail = root;
	// 	printf("1");
	// 	auto rest = (tail->up)->right;
	// 	printf("2");
	// 	while (rest != nullptr)
	// 	{
	// 		printf("3");
	// 		if ((rest->up)->left == nullptr)
	// 		{
	// 			printf("4");
	// 			tail = rest;
	// 			rest = (rest->up)->right;
	...
	// 		{
	// 			printf("5");
	// 			auto temp = (rest->up)->left;
	// 			(rest->up)->left = (temp->up)->right;
	// 			(temp->up)->right = rest;
	// 			// temp->right = rest;
	// 			rest = temp;
	// 			(tail->up)->right = temp;
	...
	{
		auto tail = RootNode();
		auto leftSubTree = Left(tail);
		auto rightSubTree = Right(tail);

		while (tail.IsNotNull())
		{
			if (leftSubTree.IsNotNull())
			{
				RotateRight(tail);
				tail = leftSubTree;
				leftSubTree = Left(tail);
				rightSubTree = Right(tail);
			}
			else
			{
				tail = rightSubTree;
	...
	{
		long long int size = root->height;
		double exponent = log((double)(size+1))/log((double)2);
		long long int leaf_count = size + 1 - pow((double)2,(double)exponent);
		Compress(leaf_count);
		size = size-leaf_count;
		while (size>1)
		{
			Compress((long long int)size/2);
			size /= 2;
		}
	}

	void Compress(long long int leaf_count)
	{
		auto rootNode = RootNode();
		for (int i=0; i<leaf_count; i++)
		{
			auto tempNode = Right(rootNode);
			RotateLeft(rootNode);
...
protected:
	void Rebalance(NodeHandle current_node)
	{
		while (current_node.IsNotNull())
		{
			
			// int rightHeight = 0;
			// int leftHeight = 0;
			// int balance = 0;
			// if (!Left(current_node).IsNull())
			// 	leftHeight = GetNode(Left(current_node))->height;
			
			// if(!Right(current_node).IsNull())
			// 	rightHeight = GetNode(Right(current_node))->height;

			int leftHeight = GetHeight(Left(current_node));
			int rightHeight = GetHeight(Right(current_node));
			...
			if (balance < -1 || balance > 1)
			{
				if(leftHeight > rightHeight)
				{
					...
					// if (!Left(Left(current_node)).IsNull())
					// 	leftLeftHeight = GetNode(Left(Left(current_node)))->height;
					
					// if (!Right(Left(current_node)).IsNull())
					// 	leftRightHeight = GetNode(Right(Left(current_node)))->height;

					int leftLeftHeight = GetHeight(Left(Left(current_node)));
					int leftRightHeight = GetHeight(Right(Left(current_node)));
				...
				else
				{
					...
					// if (!Right(Right(current_node)).IsNull())
					// 	rightRightHeight = GetNode(Right(Right(current_node)))->height;
					
					// if (!Left(Right(current_node)).IsNull())
					// 	rightLeftHeight = GetNode(Left(Right(current_node)))->height;
					
					int rightRightHeight = GetHeight(Right(Right(current_node)));
					int rightLeftHeight = GetHeight(Left(Right(current_node)));
					...
					{
						RotateRight(Right(current_node));
						RotateLeft(current_node);
						// return;
					}
				}
